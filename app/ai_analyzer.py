...
def detect_intent(question):
    """
    Ask the LLM to identify what the user wants.
    """

    prompt = f"""
You are a query classifier for a customer support ticket dataset.
...
- ticket_lookup
- escalated_tickets
- unresolved_category_analysis
- unresolved_priority_analysis
- general_question
User question:
{question}
...
def analyze_data(df, intent,ticket_id=None):
    ...
    if intent == "total_tickets":
    ...
    elif intent == "closed_tickets":

        count = len(
            df[df["status"].str.lower() == "resolved"]
        )

        return f"Closed tickets: {count}"


    elif intent == "high_priority_open":
        ...
        return f"High priority open tickets: {len(result)}"


    elif intent == "unresolved_tickets":
        ...
        tickets = result[
            ["ticket_id", "category", "priority", "status", "issue_summary"]
        ].head(10)

        return (
            f"Total unresolved tickets: {len(result)}\n\n"
            f"Sample of unresolved tickets:\n"
            f"{tickets.to_string(index=False)}"
        )
    ...
    elif intent == "unresolved_category_analysis":

        unresolved = df[
            df["status"].str.lower() != "resolved"
        ]

        result = unresolved["category"].value_counts()

        return (
            f"Unresolved tickets by category:\n\n"
            f"{result.to_string()}"
        )

    elif intent == "unresolved_priority_analysis":

        unresolved = df[
            df["status"].str.lower() != "resolved"
        ]

        result = unresolved["priority"].value_counts()

        return (
            f"Unresolved tickets by priority:\n\n"
            f"{result.to_string()}"
        )
        ...
        result = df["agent_id"].value_counts()

        return (
            f"Tickets handled by each agent:\n\n"
            f"{result.to_string()}"
        )
    elif intent == "response_time_analysis":
        category = None

        if "technical" in question.lower():
            category = "Technical"

        elif "billing" in question.lower():
            category = "Billing"

        elif "general" in question.lower():
            category = "General"
        ...
        if category:
            ...
            average_response_time = filtered_df["response_time_hrs"].mean()

            return (
                f"Average response time for {category} tickets: "
                f"{average_response_time:.2f} hours"
            )

        average_response_time = df["response_time_hrs"].mean()

        return (
            f"Overall average response time: "
            f"{average_response_time:.2f} hours"
        )

    elif intent == "resolution_time_analysis":
        if "unresolved" in question.lower():
            unresolved = df[
                df["status"].str.lower() != "resolved"
            ]

            available_times = unresolved["resolution_time_hrs"].dropna()

            if available_times.empty:
                return (
                    "There is no resolution time available for unresolved "
                    "tickets because they have not been resolved yet."
                )

            average_resolution_time = available_times.mean()

            return (
                f"Average resolution time for unresolved tickets: "
                f"{average_resolution_time:.2f} hours"
            )

        average_resolution_time = df["resolution_time_hrs"].mean()
        ...
        result = df[
            df["ticket_id"].str.upper() == ticket_id.upper()
        ]
    ...
    """
        ...
        return (
            "The question does not match one of the "
            "supported dataset analysis operations."
        )
